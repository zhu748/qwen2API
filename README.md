...
# ========== 并发控制 ==========
MAX_INFLIGHT_PER_ACCOUNT=1           # 每账号最大并发请求数（账号多时可改为 2）
...
| `MAX_INFLIGHT_PER_ACCOUNT` | `1` | 每账号最大并发数 | 账号多且稳定时可改为 `2` |
...
`MAX_INFLIGHT` 是旧版兼容别名，不建议继续使用；新配置请使用 `MAX_INFLIGHT_PER_ACCOUNT`。
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `MAX_INFLIGHT_PER_ACCOUNT` | `1` | 每个账号允许的最大并发请求数。 |
...
- `pool_acquire.py`：并发控制 + 账号获取（MAX_INFLIGHT_PER_ACCOUNT + MIN_INTERVAL + jitter）
